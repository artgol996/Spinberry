...
exports.start = start;
async function process(name, cname, videoCompression, ffmpegchaptersConfig, onProgress = function () { }) {
    let total = null;
    const metaFileName = `${name}.meta`;
    const addChaptersMeta = ffmpegchaptersConfig && await fs_1.fs.writeFile(metaFileName, ffmpegchaptersConfig).then(() => true);
    return new bluebird_1.default((resolve, reject) => {
