...
exports.detectLanguage = exports.detectFramework = void 0;
...
function detectFramework(projectPath) {
    ...
    for (const framework of frameworks_1.WIZARD_FRAMEWORKS.filter((x) => x.category === 'template')) {
        const hasAllDeps = [...framework.detectors].every((dep) => {
            return (0, frameworks_1.inPkgJson)(dep, projectPath).satisfied;
        });
    ...
    for (const library of frameworks_1.WIZARD_FRAMEWORKS.filter((x) => x.category === 'library')) {
        ...
        const hasLibrary = [...library.detectors].every((dep) => (0, frameworks_1.inPkgJson)(dep, projectPath).satisfied);
        for (const bundler of dependencies_1.WIZARD_BUNDLERS) {
            const detectBundler = (0, frameworks_1.inPkgJson)(bundler, projectPath);
