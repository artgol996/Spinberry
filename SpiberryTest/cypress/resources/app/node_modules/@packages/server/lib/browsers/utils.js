...
const plugins = (0, tslib_1.__importStar)(require("../plugins"));
...
function extendLaunchOptionsFromPlugins(launchOptions, pluginConfigResult, options) {
    ...
    if (pluginConfigResult[0]) {
        options.onWarning((0, errors_1.getError)('DEPRECATED_BEFORE_BROWSER_LAUNCH_ARGS'));
...
function ensureAndGetByNameOrPath(nameOrPath, returnAll = false, browsers = []) {
    const findBrowsers = browsers.length ? bluebird_1.default.resolve(browsers) : getBrowsers();
    return findBrowsers
        .then((browsers = []) => {
